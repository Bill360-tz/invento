        ...
        <header class="fill flex-s-btn bg-w pad-10 rad-5 shadow-pri">
            <div class="flex flex-s-btn fill">
                <div class="flex">
                    ...
                    <h1 class="theme-text disText">Items in Store</h1>
                </div>
            ...
            <div class="flex-c pad-20 flex-centered fill">
                <div class="flex-c fill bg-w rad-3 shadow-pri">
                    <div class="flex fill flex-s-btn pad-5 theme-border-b-2">
                        <div class="flex">
                            <h3 class="normal-text pad-5 ">Total = <span><?= number_format(sumStore())." Tshs." ?></span></h3>
                        </div>
                        ...
                        <table id="expRecord" class="fill">
                            <thead class="theme-text">
                                <tr>
                                    ...
                                    <th class="text-l">Date Purchased</th>
                            ...
                            <tbody>
                                ...
                                if (mysqli_num_rows(store()) > 0) {
                                    $sn = 1;
                                    foreach (store() as $exp) {
                                ?>
                                        <tr>
                                            <td><?= $sn ?></td>
                                            <td><?= $exp['item_name'] ?></td>
                                            <td><?= number_format($exp['item_cost']) ?></td>
                                            <td><?= formatDate($exp['date_purchased'])  ?></td>
                                            <td>
                                                <button onclick="editExpense('<?= $exp['item_id'] ?>', '<?= $exp['item_name'] ?>','<?= $exp['item_cost'] ?>','<?= $exp['date_purchased'] ?>')" class="btn-pri-o">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button onclick="deleteEx('<?= $exp['item_id'] ?>', '<?= $exp['item_name'] ?>')" class="btn-pri-o">
    ...
    <div class="delete_cat_model fill flex-c" style="display: none;">
        <div class="inner_delete_cat_model width-50 flex-c rad-5">
            <div class="flex flex-s-btn fill pad-5 bg-sec rad-5-t-r-5 rad-5-t-l-5">
                <h3 class="normal-text">Delete Item <span id="deletedCatNam" class="theme-text"></span></h3>
        ...
        <div class="addExpModel outer_model fill" style="display: none;">
            <div class="inner_model bg-w width-50">
                <div class="flex flex-s-btn pad-5">
                    <h3 class="theme-text">Add Product in store</h3>
                ...
                <flex-c class="flex-c fill pad-10">
                    ...
                    <div class="flex-c fill">
                        <label for="expenseDate"> Purchase date</label>
        ...
        <div class="editExpModel outer_model fill" style="display: none;">
            <div class="inner_model bg-w width-50">
                <div class="flex flex-s-btn pad-5">
                    <h3 class="theme-text">Edit store Item</h3>
                ...
                <flex-c class="flex-c fill pad-10">
                    ...
                    <div class="flex-c fill">
                        <label for="expenseDatee">Purchase date</label>
